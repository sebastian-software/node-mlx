...
public struct Qwen3Configuration: Decodable, Sendable, BaseModelConfiguration {
...
typealias Qwen3RMSNorm = RMSNorm
...
class Qwen3DecoderLayer: Module {
    ...
    init(_ config: Qwen3Configuration, layerIdx _: Int = 0) {
        _selfAttn.wrappedValue = Qwen3Attention(config)
        _mlp.wrappedValue = Qwen3MLP(config)
...
class Qwen3ModelInner: Module {
    ...
    init(_ config: Qwen3Configuration) {
        ...
        _layers.wrappedValue = (0 ..< numLayers).map { idx in Qwen3DecoderLayer(config, layerIdx: idx) }
...
public class Qwen3Model: Module, LLMModel {
    ...
    private let config: Qwen3Configuration

    public var supportsCache: Bool { true }

    public init(_ config: Qwen3Configuration) {
        self.config = config
        vocabularySize = config.vocabSize
        numLayers = config.numHiddenLayers
        numKVHeads = config.numKeyValueHeads
        headDim = config.headDim
        _model.wrappedValue = Qwen3ModelInner(config)
