...
/// Apply rotary position embeddings for GptOSS
private func gptossApplyRotaryPosEmb(
    _ q: MLXArray,
    _ k: MLXArray,
    cos: MLXArray,
    sin: MLXArray
) -> (MLXArray, MLXArray) {
    let qEmbed = (q * cos) + (gptossRotateHalf(q) * sin)
    let kEmbed = (k * cos) + (gptossRotateHalf(k) * sin)
    return (qEmbed, kEmbed)
}

/// Rotate half of the tensor for GptOSS
private func gptossRotateHalf(_ x: MLXArray) -> MLXArray {
...
// MARK: - GptOSSRMSNorm (using built-in RMSNorm)

// Note: Using MLXNN.RMSNorm directly

// MARK: - GptOSSTransformerBlock

class GptOSSTransformerBlock: Module {
    @ModuleInfo(key: "self_attn") var attention: GptOSSAttention
    let mlp: GptOSSMLP
    @ModuleInfo(key: "input_layernorm") var inputLayerNorm: RMSNorm
    @ModuleInfo(key: "post_attention_layernorm") var postAttentionLayerNorm: RMSNorm

    init(_ config: GptOSSConfiguration) {
        self._attention.wrappedValue = GptOSSAttention(config)
...
// MARK: - GptOSSModelInner

public class GptOSSModelInner: Module {
    @ModuleInfo(key: "embed_tokens") var embedTokens: Embedding
    let layers: [GptOSSTransformerBlock]
